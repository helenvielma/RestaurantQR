...
include_once "../../../Modelo/Clases/Menu.php";

/**
 * Clase locales, esta clase es la encargada de realizar las transaccciones
 * correspondientes
 */
class Locales_vista extends Conexion
{
	private $conexion;
	...
	public function getDatoMenu( $id_local )
	{
		try
		{
			...
			if ( mysql_num_rows( $result ) > 0 )
			{
				while ( $f = mysql_fetch_array($result) )
				{
					$plato = new Menu( $f[ "idMenu" ], $f[ "Nombre_Menu_es" ], $f[ "Nombre_Menu_en" ], $f[ "Precio" ] );
					array_push( $this->plato_array, $plato );
				}
			}
			else
			{
